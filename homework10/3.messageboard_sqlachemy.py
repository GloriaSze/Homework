...
# 初始化数据库连接
engine = create_engine('mysql+mysqlconnector://root:gloriasxy630@localhost:3306/homework')
...
def result(choice):
    ...
    if choice == 2:
        session = DBSession()
        id_chosen = int(input("请输入要删除的留言id："))
        message_delete = session.query(Message).filter_by(ID = id_chosen).one()
    ...
    if choice == 3:
        session = DBSession()
        id_chosen = int(input("请输入要修改的留言ID："))
        comment = input("请输入修改后的留言：")
        date = datetime.datetime.now().strftime("%Y-%m-%d %H:%M:%S")
        message_revise = session.query(Message).filter_by(ID = id_chosen).one()
    ...
    if choice == 4:
        session = DBSession()
        message = session.query(Message).filter().all()
        for i in message:
